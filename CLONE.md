...
- Promote `.96` / cut over production
- Delete binlogs
...
3. Chain `.96 → .98` needs `log_bin=ON` and `log_slave_updates=ON` **on `.96`**. If those are off, the UI blocks cloning to `.98` and does **not** restart `.94`. Apply `my.cnf` on `.96` yourself.
...
ON *.* TO 'repl'@'172.21.5.96' IDENTIFIED BY '...';
-- and from .98 to .96 when chaining
FLUSH PRIVILEGES;
```
...
CLONE_REPL_PASSWORD=...
CLONE_MYSQL_PASSWORD_CLONE_96=...   # password MySQL di .96
CLONE_MYSQL_PASSWORD_CLONE_98=...
CLONE_SSH_PASSWORD=...              # password SSH user development di .96
```
